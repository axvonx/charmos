...
static const char *slab_addr_region(vaddr_t a) {
...
static void slab_track_event(vaddr_t addr, uint64_t ra0, uint64_t ra1,
                             bool is_alloc) {
...
static void slab_track_dump(const char *label, vaddr_t addr) {
    struct slab_track *e = &slab_track_table[(addr >> 6) % SLAB_TRACK_SLOTS];
    if (e->addr != addr) {
...
static void slab_debug_assert_not_already_free(vaddr_t v, int32_t class) {
    if (class < 0)
        return;
...
static void slab_dump_corruption(void *obj, struct slab_magazine *popped_mag) {
