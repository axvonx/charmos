...
static inline cc_no_asan void set_bit(uint64_t index) {
...
static inline cc_no_asan void clear_bit(uint64_t index) {
    uint64_t byte = index / 8;
...
static inline cc_no_asan bool test_bit(uint64_t index) {
    uint64_t byte = index / 8;
    uint8_t value;
    if (byte >= BOOT_BITMAP_SIZE)
        return false;
