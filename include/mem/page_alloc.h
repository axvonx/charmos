...
#define page_alloc(n_pages, ...)                                               \
    page_alloc_full((n_pages), alloc_params_with_defaults(                     \
                                   ALLOC_FLAGS_NONE, ALLOC_BEHAVIOR_NORMAL,    \
                                   ##__VA_ARGS__))

#define page_alloc_demand(n_pages, ...)                                        \
    page_alloc_demand_full((n_pages),                                          \
                           alloc_params_with_defaults(ALLOC_FLAGS_NONE,        \
...
void *page_alloc_full(size_t n_pages, struct alloc_params params) cw_alloc();
void *page_alloc_demand_full(size_t n_pages, struct alloc_params params)
    cw_alloc();
void page_free_full(void *ptr, size_t n_pages, enum alloc_behavior behavior);
